...
public class ExpensesController {
  ...
  public ResponseEntity<List<Expense>> getAllExpenses() {
    return new ResponseEntity<>(
        expenseService.fetchAllExpenses(userSession.getAuthenticatedUserId()), HttpStatus.OK);
  }

  @GetMapping
  public ResponseEntity<List<Expense>> getAllExpensesByMonthAndYear(
      @RequestParam int month, int year) {
    return new ResponseEntity<>(
        expenseService.fetchExpensesByMonthAndYear(
            month, year, userSession.getAuthenticatedUserId()),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<List<Expense>> getExpensesByMonthAndYear(
    ...
    return new ResponseEntity<>(
        expenseService.fetchExpensesByTypeMonthAndYear(
            false, month, year, userSession.getAuthenticatedUserId()),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<List<Expense>> getIncomesByMonthAndYear(@RequestParam int month, int year) {
    return new ResponseEntity<>(
        expenseService.fetchExpensesByTypeMonthAndYear(
            true, month, year, userSession.getAuthenticatedUserId()),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<Expense> getExpensesById(@PathVariable("id") long id) {
    return new ResponseEntity<>(
        expenseService.findExpense(id, userSession.getAuthenticatedUserId()), HttpStatus.OK);
  }

  @PostMapping
  public ResponseEntity<Expense> createNewExpense(@RequestBody Expense expense) {
    return new ResponseEntity<>(
        expenseService.createExpense(expense, userSession.getAuthenticatedUserId()),
        HttpStatus.CREATED);
  }

  @PatchMapping("/{id}")
  public ResponseEntity<Expense> updateExpenseById(
      @PathVariable("id") long id, @RequestBody Map<String, Object> newData) {
    return new ResponseEntity<>(
        expenseService.updateExpense(id, newData, userSession.getAuthenticatedUserId()),
        HttpStatus.OK);
  }
